...
export function buildGuidancePrompt(input: GuidancePromptInput, onBlock?: (category: ContextCategoryKey, file?: string) => void): string {
  ...
  const automaticDecision = kind === "always" && context.additionalContext?.trim()
    ? '\n\n## Automatic decision\n現在のコード全体から実際に出力される文字列を、改行・空白・順序を含めて読み取り、課題要件と照合してください。Pythonのprintは既定で末尾に改行を出力します。文字の個数が合っていても、横一列という要件に対して複数行に出力するコードは未完成です。改行を抑制して同じ行に出力するコードは、複数のprintやループでも要件を満たせます。課題達成済みで具体的なリスクや解説意図もなければ {"kind":"no_advice","focus":"none"} だけを返してください。値を作っただけで要求された出力がない場合は未完成です。例えば「Helloを表示」という課題で message = "Hello" だけなら未完成で、print("Hello") なら完成です。不足がある場合は、現在の挙動→要件との差→着目点の順で短いヒントをcontinueで返してください。例えば出力形式の不一致なら「現在は複数行に出力されています。課題は同じ行への出力を求めています。各出力の末尾の扱いに着目してください。」のように、挙動と着目点だけを伝えます。この例文は形式の例であり、実際のコードが要件を満たしていれば助言不要です。変更後の式・引数値・コードや、特定の実装方法の指示は含めないでください。正しいコードへの実行確認・追加作業の提案は不要です。'
    : "";
