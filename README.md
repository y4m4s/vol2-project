...
- **OrcaRouter に接続**: 先に設定画面でAPIキーとモデルを設定します。

### OrcaRouterの設定

1. 設定画面の「接続先」で **OrcaRouter** を選択します。
2. `sk-orca-` で始まるAPIキーを入力し、「キーを保存」を押します。
3. 「モデル一覧を更新」で利用可能なテキストモデルを取得します。
4. モデルを選び、画面下部の「保存」を押します。
5. 初回の無課金動作確認には **Free Router (`orcarouter/free`)** を選択します。

APIキーはVS CodeのSecretStorageへ保存され、設定データや会話履歴には書き込まれません。保存済みキーの値は設定画面へ再表示されません。`orcarouter/free` は無料容量だけを利用し、容量がない場合も有料モデルへ自動移行しません。
