...
ブランチ`feature/adaptive-provider-routing`では、圧縮検証を含む先行PRを基準として次を実装する。

- バージョン1の初期評価表と、用途別の重み付け
- 質問、推論強度、スラッシュコマンド、診断、対象ファイル数、変更数だけを使う決定的なタスク分類
- 正常回答10回の学習ゲートと、候補1件時の待機
- 初期評価10件分と端末内の実績を合成する採点
- 8点の切り替え基準、2ターンのクールダウン、低リスク時だけ5回に1回までの探索
- 成功、接続失敗、形式修復、タイムアウト、応答時間、Good／Badを保存する`provider-routing.sqlite`
- 学習回数と自動選択の有効状態を設定画面へ表示
- 本文を含まない`provider_route_evaluated`診断

初期評価は絶対的なモデル順位ではなく、実測がない状態での保守的な開始値である。通常回答を保存できた場合だけ学習回数を増やし、圧縮、接続確認、形式修復の追加通信は回数へ含めない。OrcaRouterが実モデルIDを返した場合はルーター全体と実モデルの双方へ実績を記録するが、学習回数は1回だけ増やす。

このPRでは評価値の削除UIと評価表の詳細表示は追加しない。初期評価表の再較正には、固定問題によるモデル評価結果を別途用いる。
...
実装済み。質問本文、回答本文、要約、ファイル本文、フィードバックコメントは評価DBへ保存しない。
...
コードと自動テストは実装済み。F5で2件以上の実プロバイダーを使う手動確認は未実施。
