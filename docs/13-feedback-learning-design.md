# 13. アドバイス評価・フィードバック学習機能 旧設計

> この文書は、評価理由をAIで要約していた旧設計の記録です。現在は評価時の外部送信と追加トークン消費を避けるため、理由をローカルの定型傾向へ変換します。現行仕様は [16-feedback-implementation.md](./16-feedback-implementation.md) を参照してください。
