...
OrcaRouter利用時、回答生成、会話タイトル生成、ナレッジ作成などのプロンプトはOrcaRouterと上流モデル事業者へ送信される。質問やコード断片だけでなく、処理に応じて診断情報、ファイル名・ディレクトリ構造、追加コンテキスト、会話内容、再利用ナレッジのタイトル・要約、評価理由からローカル生成した定型フィードバック傾向も含まれる。補足コメントはフィードバック処理では送信しない。READMEと設定画面でこれを明示する。既存の保護済み・追加除外globはOrcaRouterでもファイル本文に適用し、APIキーはプロンプトへ含めない。
