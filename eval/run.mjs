...
const rubric=readFileSync('eval/rubric.md','utf8')+(v.suite?.startsWith('algorithms')?'\n\n'+readFileSync('eval/algorithm-rubric.md','utf8'):'');
const manifest = {schemaVersion:1,experimentId:config.id,config,suite:v.suite ?? 'original',split:v.split,repeats,caseIds:cases.map(x=>x.id),caseSetHash:hash(cases),provenance:provenance(),...(v.suite?.startsWith('algorithms')?{algorithmProvenance:algorithmProvenance()}:{}),rubricHash:hash(rubric),startedAt:new Date().toISOString(),orderSeed,dry:v.dry};
