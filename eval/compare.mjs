...
for(const a of runs[0].responses) {
  ...
  const pair = {pairId,task:{input:item.input,expected:item.expected,category:item.category},A:anonymous(swapped?b:a),B:anonymous(swapped?a:b)};
  pairs.push(pair);key.push({pairId,caseId:a.caseId,repeat:a.repeat,candidateSide:swapped?'A':'B',pairHash:hash(pair)});
}
newDirectory(v.out);
const packet={schemaVersion:1,rubric:readFileSync('eval/rubric.md','utf8'),pairs:shuffle(pairs,seed)};
