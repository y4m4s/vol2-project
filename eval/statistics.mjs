...
export function bootstrapCases(rows,seed='bootstrap-v1',iterations=5000) {
  const groups=[...new Set(rows.map(x=>x.caseId))].map(id=>mean(rows.filter(x=>x.caseId===id).map(x=>x.win)));
  ...
  return {low:percentile(draws,.025),high:percentile(draws,.975),unit:'case',cases:groups.length,iterations};
}
...
export function summarizeJudgments(result,key) {
  ...
  return {pairwiseWinRate:mean(rows.map(x=>x.win)),decisiveWinRate:mean(rows.filter(x=>x.win!==.5).map(x=>x.win)),ties:rows.filter(x=>x.win===.5).length,ci95:bootstrapCases(rows),
